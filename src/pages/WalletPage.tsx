...
export default function WalletPage() {
  ...
  return (
    <div className="min-h-screen bg-sovereign-surface/50 animate-fade-in pb-24">
      <div className="glass sticky top-0 z-10 px-4 py-3 flex items-center justify-between border-b border-glass-border/50">
        <div />
        <h1 className="text-lg font-extrabold text-text-primary">محفظتي</h1>
      ...
      <div className="px-4 pt-6">
        {/* Main Balance Card */}
        <div className="bg-gradient-to-l from-gray-900 to-gray-800 rounded-3xl p-6 shadow-sovereign-xl relative overflow-hidden animate-fade-in-up">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-text-tertiary">{state.user?.name || 'متجر العراق'}</span>
            ...
            <div className="text-right">
              <p className="text-[11px] text-text-secondary mb-1">الرصيد المتاح</p>
              <p className="text-3xl font-black text-white">IQD {(state.user?.walletBalance || 0).toLocaleString()}</p>
            </div>

            <div className="flex gap-3 mt-6">
              <button onClick={() => navigate('/transactions')} className="flex-1 bg-sovereign-card/10 rounded-xl py-3 flex items-center justify-center gap-2 hover:bg-sovereign-card/15 transition-all btn-press border border-white/10">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4 stagger-children">
          <div className="bg-sovereign-card rounded-2xl p-4 shadow-sovereign border border-glass-border/80 animate-fade-in-up">
            <div className="flex items-center gap-2 justify-end mb-2">
              <span className="text-xs font-bold text-text-secondary">الإيداعات</span>
            ...
            <p className="text-right text-[10px] text-text-tertiary">دينار عراقي</p>
          </div>
          <div className="bg-sovereign-card rounded-2xl p-4 shadow-sovereign border border-glass-border/80 animate-fade-in-up">
            <div className="flex items-center gap-2 justify-end mb-2">
              <span className="text-xs font-bold text-text-secondary">المصروفات</span>
              <div className="w-8 h-8 bg-sovereign-card rounded-lg flex items-center justify-center">
        ...
        {showAdd && (
          <div className="bg-sovereign-card rounded-2xl p-5 mt-4 shadow-sovereign border border-glass-border/80 animate-slide-up">
            <h3 className="font-extrabold text-text-primary mb-4 text-right">إضافة رصيد</h3>
                ...
                <button
                  ...
                  className={`py-2.5 rounded-xl text-xs font-bold border-2 transition-all duration-300 btn-press ${
                    amount === val.toString()
                      ...
                      : 'bg-sovereign-surface border-glass-border text-text-secondary hover:border-sovereign-border'
                  }`}
                >
                  {val.toLocaleString()}
                </button>
            ...
            <input
              ...
              type="number"
              className="w-full h-12 bg-sovereign-surface rounded-xl px-4 text-sm text-right outline-none border-2 border-glass-border focus:border-emerald-400 focus:ring-4 focus:ring-emerald-50 mb-4 transition-all"
            />
            <button
