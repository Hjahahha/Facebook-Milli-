...
export default function AccountPage() {
  ...
  if (!state.isLoggedIn) {
    return (
      <div className="pb-24 animate-fade-in min-h-screen bg-sovereign-card">
        <div className="flex flex-col items-center justify-center py-20 px-8 animate-fade-in-up">
          <div className="w-28 h-28 bg-gradient-to-br from-gray-100 to-gray-50 rounded-3xl flex items-center justify-center mb-6 shadow-sovereign">
            <User size={48} className="text-text-tertiary" />
          </div>
          <h2 className="text-2xl font-black text-text-primary mb-2">مرحباً بك</h2>
          <p className="text-text-tertiary text-sm text-center mb-8 font-medium">سجل دخول للوصول لجميع المميزات</p>
          ...
          <button onClick={() => navigate('/register')} className="w-full mt-3 border-2 border-neon text-neon py-3.5 rounded-2xl font-bold text-sm hover:bg-sovereign-card transition-all btn-press">
            إنشاء حساب جديد
          </button>
        </div>
      </div>
  ...
  return (
    <div className="pb-24 animate-fade-in bg-sovereign-surface/50 min-h-screen">
      {/* Profile Header */}
      <div className="bg-sovereign-card relative overflow-hidden">
        ...
        <div className="relative z-10 px-4 pt-6 pb-5">
          <div className="flex items-center gap-4 justify-end">
            <div className="text-right">
              <h2 className="text-lg font-black text-text-primary">{state.user?.name}</h2>
              <p className="text-sm text-text-tertiary font-medium">{state.user?.phone}</p>
            </div>
          ...
          {/* Wallet & Points Card */}
          <div className="mt-4 bg-gradient-to-l from-gray-900 to-gray-800 rounded-2xl p-4 flex items-center justify-between shadow-sovereign-lg relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              <div className="bg-sovereign-card/10 rounded-xl px-3 py-1.5">
                <span className="text-[10px] text-text-tertiary block">نقاط</span>
            ...
            <div className="flex items-center gap-3 relative z-10">
              <div className="text-right">
                <span className="text-[10px] text-text-tertiary block">محفظة</span>
                <span className="text-white font-bold text-sm">IQD {(state.user?.walletBalance || 0).toLocaleString()}</span>
              </div>
              <div className="w-10 h-10 bg-sovereign-card/10 rounded-xl flex items-center justify-center">
          ...
          <div className="mt-4 grid grid-cols-4 gap-2.5 stagger-children">
            {quickActions.map(item => {
              ...
              return (
                ...
                >
                  <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${item.color} flex items-center justify-center shadow-sovereign`}>
      ...
      <div className="mt-2 space-y-2">
        {menuSections.map((section, sIdx) => (
          <div key={sIdx} className="bg-sovereign-card">
            <div className="px-4 pt-4 pb-2">
              <h3 className="text-xs font-bold text-text-tertiary text-right uppercase tracking-wider">{section.title}</h3>
            </div>
            ...
            {section.items.map(item => {
              ...
              return (
                <button
                  key={item.path}
                  onClick={() => navigate(item.path)}
                  className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-sovereign-surface transition-all duration-200 btn-press"
                >
                  <ChevronLeft size={16} className="text-text-tertiary" />
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-semibold text-text-primary">{item.label}</span>
                    <div className="w-8 h-8 bg-sovereign-surface rounded-lg flex items-center justify-center">
        ...
        <div className="bg-sovereign-card">
          <button
            onClick={() => navigate('/merchant-join')}
            className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-sovereign-surface transition-all btn-press"
          >
            <ChevronLeft size={16} className="text-text-tertiary" />
            <div className="flex items-center gap-3">
        ...
        <div className="bg-sovereign-card">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-4 text-neon hover:bg-sovereign-card transition-all btn-press"
          >
      ...
      <div className="fixed bottom-20 left-4 z-40">
        <button className="bg-gray-900 text-white rounded-2xl px-4 py-3 flex items-center gap-2 shadow-sovereign-xl hover:bg-gray-800 transition-all btn-press">
