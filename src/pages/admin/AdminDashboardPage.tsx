    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-4">هذه الصفحة متاحة فقط لمدير التطبيق</p>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 whitespace-nowrap px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                  activeTab === item.id ? 'bg-white text-gray-900' : 'text-gray-400 hover:text-white hover:bg-gray-800'
                }`}
              >
                <Icon size={14} />
...
function OverviewTab({ stats }: { stats: { label: string; value: string; icon: typeof Users; color: string; change: string }[] }) {
  return (
    <div className="animate-fade-in">
      ...
      {/* Quick Actions */}
      <h3 className="font-bold text-gray-800 mb-3 text-right">إجراءات سريعة</h3>
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'التقارير', icon: TrendingUp, color: 'text-red-600' },
        ].map(action => {
          const Icon = action.icon;
          return (
            <button key={action.label} className="bg-white rounded-xl p-3 text-center shadow-sm border border-gray-100 hover:shadow-md transition-all">
              <Icon size={24} className={`${action.color} mx-auto mb-1`} />
              <p className="text-[10px] text-gray-700 font-medium">{action.label}</p>
            </button>
          );
        })}
      </div>
      ...
      {/* Recent Activity */}
      <h3 className="font-bold text-gray-800 mt-6 mb-3 text-right">النشاط الأخير</h3>
...
function MerchantsTab() {
  ...
  return (
    <div className="animate-fade-in">
      <h3 className="font-bold text-gray-800 mb-3 text-right">طلبات انضمام التجار ({state.merchantApplications.length})</h3>
            ...
            <div key={app.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500">{app.userName} - {app.userPhone}</p>
                </div>
              </div>
              <div className="text-xs text-gray-600 space-y-1 mb-3">
                <p className="text-right">النوع: {app.businessType}</p>
                <p className="text-right">المدينة: {app.city}</p>
                <p className="text-right">الباقة: <span className={app.tier === 'premium' ? 'text-yellow-600 font-bold' : 'text-gray-600'}>{app.tier === 'premium' ? 'مميزة (99,000 دينار)' : 'عادية (25,000 دينار)'}</span></p>
...
function ProductsTab() {
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-3">
        <button onClick={() => setShowAdd(!showAdd)} className="bg-red-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1">
          <PlusCircle size={14} /> إضافة منتج
        </button>
        <h3 className="font-bold text-gray-800">المنتجات ({state.products.length})</h3>
      </div>

      {showAdd && (
        <div className="bg-white rounded-xl p-4 mb-4 shadow-sm border border-gray-200 space-y-3 animate-slide-up">
          <input value={newProduct.name} onChange={e => setNewProduct({ ...newProduct, name: e.target.value })} placeholder="اسم المنتج" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
          <input value={newProduct.price} onChange={e => setNewProduct({ ...newProduct, price: e.target.value })} placeholder="السعر (دينار)" type="number" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
          <select value={newProduct.category} onChange={e => setNewProduct({ ...newProduct, category: e.target.value })} className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border">
          ...
          <textarea value={newProduct.description} onChange={e => setNewProduct({ ...newProduct, description: e.target.value })} placeholder="وصف المنتج" className="w-full h-20 bg-gray-50 rounded-lg p-3 text-sm text-right outline-none border resize-none" />
      ...
      <div className="space-y-2">
        {state.products.map(p => (
          <div key={p.id} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 flex items-center gap-3">
            <button onClick={() => dispatch({ type: 'REMOVE_PRODUCT', payload: p.id })} className="text-red-400 hover:text-red-600">
              <Trash2 size={16} />
            </button>
            <div className="flex-1 text-right">
              <p className="text-sm font-semibold line-clamp-1">{p.name}</p>
              <p className="text-xs text-red-600 font-bold">{p.price.toLocaleString()} دينار</p>
            </div>
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-3">
        ...
        <h3 className="font-bold text-gray-800">الفئات ({state.categories.length})</h3>
      </div>

      {showAdd && (
        <div className="bg-white rounded-xl p-4 mb-4 shadow-sm border border-gray-200 space-y-3 animate-slide-up">
          <input value={newCat.name} onChange={e => setNewCat({ ...newCat, name: e.target.value })} placeholder="اسم الفئة" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
          <input value={newCat.icon} onChange={e => setNewCat({ ...newCat, icon: e.target.value })} placeholder="أيقونة (إيموجي)" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
      ...
      <div className="space-y-2">
        {state.categories.map(c => (
          <div key={c.id} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 flex items-center justify-between">
            <button onClick={() => dispatch({ type: 'REMOVE_CATEGORY', payload: c.id })} className="text-red-400 hover:text-red-600">
              <Trash2 size={16} />
            </button>
            <div className="flex items-center gap-3">
              <div className="text-right">
                <p className="text-sm font-semibold">{c.name}</p>
                <p className="text-xs text-gray-500">{c.subcategories.length} فئة فرعية</p>
              </div>
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-3">
        ...
        <h3 className="font-bold text-gray-800">الإعلانات ({state.ads.length})</h3>
      </div>

      {showAdd && (
        <div className="bg-white rounded-xl p-4 mb-4 shadow-sm border border-gray-200 space-y-3 animate-slide-up">
          <input value={newAd.title} onChange={e => setNewAd({ ...newAd, title: e.target.value })} placeholder="عنوان الإعلان" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
          <input value={newAd.image} onChange={e => setNewAd({ ...newAd, image: e.target.value })} placeholder="رابط الصورة" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
          <input value={newAd.link} onChange={e => setNewAd({ ...newAd, link: e.target.value })} placeholder="رابط الإعلان (اختياري)" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
          ...
          <div key={ad.id} className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100">
            ...
            <div className="p-3 flex items-center justify-between">
              <div className="flex gap-2">
                ...
                <button onClick={() => dispatch({ type: 'TOGGLE_AD', payload: ad.id })} className={ad.active ? 'text-green-500' : 'text-gray-400'}>
                  {ad.active ? <Eye size={16} /> : <EyeOff size={16} />}
                </button>
              </div>
              <div className="text-right">
                <p className="text-sm font-semibold">{ad.title}</p>
                <p className="text-[10px] text-gray-500">{ad.active ? 'نشط' : 'متوقف'}</p>
              </div>
            </div>
          </div>
        ))}
...
function OrdersTab() {
  ...
  return (
    <div className="animate-fade-in">
      <h3 className="font-bold text-gray-800 mb-3 text-right">الطلبات ({state.orders.length})</h3>
          ...
          {state.orders.map(order => (
            <div key={order.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-sm font-bold text-gray-800">#{order.id.slice(0, 6)}</p>
              </div>
              <p className="text-xs text-gray-500 text-right">{order.items.length} منتج - {order.total.toLocaleString()} دينار</p>
              <p className="text-[10px] text-gray-400 text-right mt-1">{formatDate(order.createdAt)}</p>
              <div className="flex gap-2 mt-3">
                {['confirmed', 'shipped', 'delivered'].map(s => (
                  <button
                    ...
                    className="flex-1 bg-gray-50 text-gray-600 py-1.5 rounded-lg text-[10px] font-bold hover:bg-gray-100"
                  >
                    {getStatusText(s)}
                  </button>
                ))}
...
function UsersTab() {
  return (
    <div className="animate-fade-in">
      <h3 className="font-bold text-gray-800 mb-3 text-right">المستخدمين</h3>
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y">
        {[
...
function NotificationsTab() {
  ...
  return (
    <div className="animate-fade-in">
      <h3 className="font-bold text-gray-800 mb-3 text-right">إرسال إشعار جماعي</h3>
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 space-y-3">
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="عنوان الإشعار" className="w-full h-10 bg-gray-50 rounded-lg px-3 text-sm text-right outline-none border" />
        <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="نص الإشعار" className="w-full h-24 bg-gray-50 rounded-lg p-3 text-sm text-right outline-none border resize-none" />
        <div className="flex gap-2">
          <button className="flex-1 bg-gray-100 text-gray-700 py-2.5 rounded-lg text-sm font-bold">للتجار فقط</button>
...
function SettingsTab() {
  return (
    <div className="animate-fade-in">
      <h3 className="font-bold text-gray-800 mb-3 text-right">إعدادات التطبيق</h3>
      <div className="space-y-3">
        {[
          ...
          return (
            <div key={setting.label} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
