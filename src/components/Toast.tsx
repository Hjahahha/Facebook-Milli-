...
export default function Toast({ message, type = 'success', onClose, duration = 3000 }: ToastProps) {
  ...
  const colors = {
    ...
    error: 'bg-sovereign-card border-red-200 text-red-800',
    info: 'bg-blue-50 border-blue-200 text-blue-800',
  };
  const iconColors = { success: 'text-emerald-500', error: 'text-error', info: 'text-blue-500' };
  ...
  return (
    <div className={`fixed top-4 left-4 right-4 z-[100] max-w-lg mx-auto transition-all duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
      <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border shadow-sovereign-lg ${colors[type]}`}>
